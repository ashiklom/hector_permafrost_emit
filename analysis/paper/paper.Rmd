...
where $a$, $b$, and $c$ are model parameters whose values are described in the _Configuration_ section below.
...
In the original formulation, all of the total heterotrophic respiration was assumed to be CO~2~ and natural CH~4~ emissions were entirely separate.
In the revised version here, total heterotrophic respiration is divided into CO~2~ ($RH_{CO_2}[i,t]$) and CH~4~ ($RH_{CH_4}[i,t]$) based on the biome-specific CH~4~ fraction parameter ($f_{CH_4}[i]$):

\[ RH_{CH_4}[i,t] = f_{CH_4}[i] RH[i,t] \]

\[ RH_{CO_2}[i,t] = (1 - f_{CH_4}[i]) RH[i,t] \]
